...
| **Dedications on screen** | Whoever queues a song can send a line with it — *"for Ana, who turns 30 today"* — and change it for as long as the song is waiting. It takes its turn across the top of the television while the song plays, white on a strip dark enough to read it against, and then gives the screen back to the lyrics. An admin can put a message on any song playing or queued, edit or take down anyone's, and turn the whole display off for a room — nothing is deleted, and it all comes back when it is turned on again. |
