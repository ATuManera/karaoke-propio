...
class Prefs {
  ...
  static get () {
    const prefs = {
      ...
      // What the Player's QR should encode. Guests may be on mobile data
      // rather than the LAN, so the invite must carry the public name instead
      // of whatever address the Player itself happens to be open at.
      publicUrl: (process.env.KES_PUBLIC_URL ?? '').replace(/\/+$/, ''),
