...
// log non-default settings — but never secrets (Karaoke Propio added
// USDB_PASSWORD to `env`; it must never reach the logs, console or file)
const SENSITIVE_ENV_KEYS = new Set(['USDB_PASSWORD'])
for (const key in env) {
  if (SENSITIVE_ENV_KEYS.has(key)) {
    if (process.env[key]) log.verbose(`${key}=***`)
    continue
  }
...
;(async function () {
  ...
  // Karaoke Propio: init pitch shifting manager and rebuild its in-memory
  // state (preparing/ready/error is never persisted as durable truth) BEFORE
  // the web server starts accepting connections, so no client ever observes
  // a queue row stuck in limbo after a restart.
  const { default: PitchManager } = await import('./Pitch/PitchManager.js')
  PitchManager.init({
    cacheDir: env.KES_PITCH_CACHE_DIR,
    workerUrl: env.KES_PITCH_WORKER_URL,
    maxConcurrency: env.KES_PITCH_MAX_CONCURRENCY,
  })
  await PitchManager.reconcile()
