...
/**
 * Identifies the deployed client build. Derived from the built index.html,
 * which references content-hashed bundles, so it changes exactly when the
 * client code changes — restarting the server without rebuilding keeps the
 * same id and does not disturb anyone.
 */
...
function getBuildId (): string {
  ...
  try {
    ...
    buildId = crypto.createHash('sha1').update(fs.readFileSync(indexPath)).digest('hex').slice(0, 12)
  } catch {
    // no built client (dev server); a constant keeps clients from reloading
    buildId = 'dev'
  }
...
export default function (io, jwtKey) {
  io.on('connection', (sock) => {
    // Tell every client which build this server is serving. A client compares
    // it against the one it connected with and reloads if it changed — the
    // Player is typically left open on a TV for days, so after a deploy it
    // keeps running old code while appearing perfectly healthy. That failure
    // is invisible: everything already loaded still works, only newly added
    // commands are silently ignored.
    sock.emit('buildId', getBuildId())
