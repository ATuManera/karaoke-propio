...
class CDGPlayer extends React.Component<CDGPlayerProps> {
  ...
  componentDidUpdate (prevProps: CDGPlayerProps) {
    ...
    // a new nonce means a fresh seek request, even to the same position
    if (prevProps.seekNonce !== this.props.seekNonce && typeof this.props.seekPosition === 'number') {
  ...
  updateSources = async () => {
    ...
    // load .cdg file
    try {
      const response = await api.get(`/${this.props.mediaId}?type=cdg&queueId=${this.props.mediaKey}`)
      ...
      this.audio.current.src = `${document.baseURI}api/media/${this.props.mediaId}?type=audio&queueId=${this.props.mediaKey}`
