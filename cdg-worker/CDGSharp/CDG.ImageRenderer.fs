...
let private writeExplanation (ctx: IImageProcessingContext) text =
    ctx.DrawText(text, SystemFonts.CreateFont("DejaVu Sans", 10f), Color.Black, Point(10, 10)) |> ignore
...
let private writeTime (ctx: IImageProcessingContext) index =
    ...
    ctx.DrawText(time.ToString(), SystemFonts.CreateFont("DejaVu Sans", 10f), Color.Black, Point(x, 10)) |> ignore
