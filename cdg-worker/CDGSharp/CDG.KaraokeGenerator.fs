...
module KaraokeGenerator =
    ...
    let private renderText text foregroundColor backgroundColor =
        ImageSharpImageProcessing.renderText text.Content text.Font.Type text.Font.Size text.Font.Style foregroundColor backgroundColor
    ...
    let private renderTiledLines lines font x y lineHeight foregroundColor backgroundColor =
        ...
        let sliceRenderedText text xStart xEnd (renderedText: RenderedText<Color option>) : RenderedText<Color option> =
            let width = Math.Max(xEnd - xStart, 0)
            let height = RenderedText.height renderedText
            if width = 0 || height = 0 then
                RenderedText.init text (0, 0) (fun _ _ -> None)
            else
                RenderedText.init text (width, height) (fun x y ->
                    match RenderedText.tryGet (xStart + x) y renderedText with
        ...
        |> List.mapi (fun i lineParts ->
            let fullText =
                lineParts
                |> List.map (fun part -> part.Text)
                |> String.concat ""

            let fullRenderedText =
                renderText { Content = fullText; Font = font } foregroundColor backgroundColor

            let fullWidth = RenderedText.width fullRenderedText
            let offsetX = Position.getOffsetStart fullWidth Display.contentWidth x

            let partsWithBounds =
                (([], "", 0), lineParts)
                ||> List.fold (fun (parts, prefix, xStart) linePart ->
                    if linePart.Text = "" then
                        ((linePart, xStart, xStart) :: parts, prefix, xStart)
                    else
                        let nextPrefix = prefix + linePart.Text
                        let prefixRendered =
                            renderText { Content = nextPrefix; Font = font } foregroundColor backgroundColor
                        let xEnd = Math.Min(RenderedText.width prefixRendered, fullWidth)
                        ((linePart, xStart, xEnd) :: parts, nextPrefix, xEnd)
                )
                |> fun (parts, _, _) -> List.rev parts

            partsWithBounds
            |> List.map (fun (linePart, xStart, xEnd) ->
                let renderedPart =
                    sliceRenderedText linePart.Text xStart xEnd fullRenderedText

                let partTiles =
                    TilesColors.generateTiles
                        (OffsetStart (offsetX + xStart))
                        (OffsetStart (offsetY + i * lineHeight))
                        renderedPart

                (partTiles, linePart.Duration)
            )
        )
