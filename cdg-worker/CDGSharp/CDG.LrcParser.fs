...
module LyricsLine =
    let private wordPattern =
        @"(?<start>\[\d{2}:\d{2}:\d{2}\])(?<text>.*?)(?<end>\[\d{2}:\d{2}:\d{2}\])"

    let parse text =
        let matches = Regex.Matches(text, wordPattern) |> Seq.cast<Match> |> Seq.toList
        if matches |> List.isEmpty then
            failwithf "Can't parse line \"%s\"" text
        ...
        if reconstructed <> text then
            failwithf "Can't parse complete line \"%s\"" text

        matches
        |> List.map (fun m -> LyricsWord.parse m.Value)
